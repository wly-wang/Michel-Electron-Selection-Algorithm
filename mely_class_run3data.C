#define mely_class_run3data_cxx
#include "mely_class_run3data.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <vector>
...
//by  b_branchname->GetEntry(ientry); //read only this branch
    ...
    TProfile* h_mu_ly_reco_x = new TProfile("h_mu_ly_reco_x","Total Light Yield Measurement Using Reconstructed MC Muons (Run3)",10,-1,257,"");
    ...
    greater_than_50.open("michel_greater_than_50mev_run3.txt", ios::app | ios::out);
    
    //create and open txt file for storing information of all the selected michel electron candidates
    fstream all_michel;
    all_michel.open("all_michel_run3.txt", ios::app | ios::out);
   ...
   for (Long64_t jentry=0; jentry<50000.;jentry++)
   {
      ...
      nb = fChain->GetEntry(jentry);   nbytes += nb;
           ...
           itrack<(*trk_sce_start_x_v).size(); itrack++)
       {
           ...
           float stposx, stposy, stposz, endposx, endposy, endposz, trklen, trk_score, trk_pid, trk_recoE_y;
       ...
       if(GoodMuExists)
       {
               ...
               itrack<(*trk_sce_start_x_v).size(); itrack++)
           {
               ...
               //define local variables inside the loop for computing and calculation usage.
               float stposx, stposy, stposz, endposx, endposy, endposz, trklen, trk_score, trk_pid, trk_recoE_u, trk_recoE_v, trk_recoE_y, val_mu_endposx, val_mu_endposy, val_mu_endposz;
               stposx = (*trk_sce_start_x_v)[itrack];
               stposy = (*trk_sce_start_y_v)[itrack];
               stposz = (*trk_sce_start_z_v)[itrack];
               endposx = (*trk_sce_end_x_v)[itrack];
                       ...
                       if(mu_michel_gap<10. && trk_score<0.5)
                       {
                           ...
                           if (michel_flash_trk_dist<160. && calc_devi_flash_trk(flash_zcenter,flash_zwidth,stposz,endposz)<=1.)
                           {
                               ...
                               if(tot_pe_per_trk>0 && trk_recoE_y>0)
                               {
                                   ...
                                   if(trk_recoE_y>50)
                                   {
                                       greater_than_50 << left << setw(10) << run << setw(10) << sub << setw(10) << evt << setw(15) << stposx << setw(15) << stposy << setw(15) << stposz << setw(15) << endposx << setw(15) << endposy << setw(15) << endposz << setw(15) << trk_score << setw(15) << mu_michel_gap << setw(10) << trk_recoE_y << endl;
                                   }
    ...
    unique_ptr<TFile> myFile(TFile::Open("run3_analyz_outputs.root", "RECREATE"));
    ...
    //print to .root file and pdf
    c25 -> Print("plots.ps");
    myFile -> WriteObject(c25, "Run3 Muon Reconstructed Light Yield Measurement (x)");
    ...
    //print to .root file and pdf
    c27 -> Print("plots.ps");
    myFile -> WriteObject(c27, "Run3 Michel Reconstructed Light Yield Measurement (x)");
